...
class Docker:
    def __init__(self):
        ...
        if self.is_in_docker():
            sys.exit('Do not execute this from within the container.  Run it on the host, instead.')

    def is_in_docker(self):
        if os.path.exists('/.dockerenv'):
            return True
        _path = '/proc/self/cgroup'
        if os.path.isfile(_path) and any('docker' in line for line in open(_path)):
            return True
    ...
    def prune(self):
        for fullname in self.iterate_files():
            ...
            if self.file_date(fullname) < self.earliest_date():
                delete_cmd = f"docker exec -t {self.id} rm -rf {fullname}"
                print(f"Directory {filename} too old.  Deleting: {delete_cmd}")
